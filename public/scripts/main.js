...
function slideItem(carousel, arrowBtns, firstCardWidth, childrenCarousel, wrapper, gap, idLeft, idRight = "") {
  let cardPreview = Math.round(carousel.offsetWidth / firstCardWidth),
    timeOutId;
  childrenCarousel.slice(-cardPreview).reverse().forEach(item => {
  ...
  const autoPlay = () => {
    ...
    timeOutId = setTimeout(() => carousel.scrollLeft += firstCardWidth + 24, 2500);
  };
  autoPlay();
  const infiniteScroll = () => {
    ...
    } else if (Math.ceil(carousel.scrollLeft) === carousel.scrollWidth - carousel.offsetWidth + 24) {
      carousel.classList.add("no-transition");
  ...
  arrowBtns.forEach(item => {
    item.addEventListener("click", function () {
      carousel.scrollLeft += item.id === idLeft ? -firstCardWidth - 24 : firstCardWidth + 24;
    });
  });
...
slideItem(carousel, arrowBtns, firstCardWidth, childrenCarousel, wrapper, "left");
...
slideItem(popularList, popularArrowBtns, firstCardPopular, childrenPopular, popularItems, "left");
...
slideItem(trendingList, trendingArrowBtns, firstCardTrending, childrenTrending, trendingItems, "left");